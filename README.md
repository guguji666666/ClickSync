...
- 使用罗技鼠标时，网页驱动只有在板载内存模式下才能写入并使用设备配置。可提前在 GHUB 中开启；若连接时检测到未开启，本驱动会询问是否自动开启：点击“确定”表示开启并进入，点击“取消”表示不启用并继续进入。未适配型号可能因板载配置为空出现按键不可用等异常，请谨慎开启；若出现异常，关闭板载内存模式即可。
