...
- 使用罗技鼠标时，本驱动会自动开启板载内存模式，使板载配置生效；开启后 GHUB 将无法接管设备。如需使用 GHUB，请先关闭板载内存模式。不受支持的罗技型号可能因缺少板载配置，出现按键不可用等异常，请谨慎尝试；如已出现问题，可关闭板载内存模式，或通过 GHUB 重新写入默认配置。
...
- When using Logitech mice, this driver automatically enables Onboard Memory Mode so the onboard profile takes effect. While enabled, GHUB cannot control the device. To use GHUB, first turn off Onboard Memory Mode. Unsupported Logitech models may be missing onboard profiles and can have issues such as unavailable buttons; proceed with caution. If issues occur, turn off Onboard Memory Mode or use GHUB to rewrite the default profile.
